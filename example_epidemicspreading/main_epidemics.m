...
%                   (note that A(i,j) > 0 if i -> j, differently)
...
% code are irrelevant, only the difference between zero and nonzero
...
targetcities = [19 66 68 19 67 75 74 38 72 35 14 41 36 60 8];
...
S = find_msp_depth(Astruct,T,Cand,2);      % minimum sensor placement
...
[foN,foJ,foH,foD,foE,foL] = functobsv_design_v2(A,C,F0);
...
disp(['Note that predictions made based on free simulation are very',...
      ' dependent on the random choice of city for the epidemic',...
      ' outbreak and thus can achieve better or close performance (to',...
      ' the simulations made with functional observers)',...
